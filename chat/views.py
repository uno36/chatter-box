...
from account.forms import AddUserForm, EditUserForm, RemoveUserForm
...
def remove_user(request, user_id):
    ...
    if request.method == 'POST':
        form = RemoveUserForm(request.POST, instance=user_to_remove)

        if form.is_valid():
            user_to_remove.delete()
            messages.success(
                request, f'The user {user_to_remove.username} was removed!')
            return redirect('/chat-admin/')  # Redirect to the appropriate page
    else:
        form = RemoveUserForm(instance=user_to_remove)

    return render(request, 'chat/remove_user.html', {'form': form, 'user': user_to_remove})
